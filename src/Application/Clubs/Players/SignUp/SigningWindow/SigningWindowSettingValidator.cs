using Application.Validators;
using FluentValidation;

namespace Application.Clubs.Players.SignUp.SigningWindow;

public class SigningWindowSettingValidator : DateRangeValidator<SigningWindowSetting>
{
    public const int MaximunmWindowTimeInDays = 180;

    public SigningWindowSettingValidator()
    {
        RuleFor(s => s)
            .Must(s => (s.End - s.Start).TotalDays <= MaximunmWindowTimeInDays)
            .WithMessage($"Signing window can not exceed {MaximunmWindowTimeInDays} days");
    }
}
