...
internal sealed class SignUpPlayerCommandHandler
    (IResultRepository<(ClubId ClubId, PlayerId PlayerId), SignUpPlayerAggregate, SignUpPlayerAggregateRoot> repository,
            ...
            : ICommandHandler<SignUpPlayerCommand>
{
    public async Task<Result> HandleAsync(SignUpPlayerCommand command, CancellationToken ct)
    {
        ...
        var signingDateRange = DateRange.From(signingWindow.Start, signingWindow.End);

        Result result = aggregate.SignUp(command.ContractDuration,
            command.AnualSalary, signingDateRange, dateTimeProvider);
