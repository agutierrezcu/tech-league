...
internal sealed class ReleasePlayerRepository
    (TechLeagueDbContext dbContext)
        : ResultRootedRepositoryBase
            <(ClubId ClubId, PlayerId PlayerId), ReleasePlayerAggregate, PlayerContract>
                (dbContext)
{
            ...
            CancellationToken ct)
    {
        PlayerContract playerContract = await dbContext.Contracts
            ...
            .FirstOrDefaultAsync(c => c.PlayerId == specification.PlayerId &&
                c.ClubId == specification.ClubId, ct);
