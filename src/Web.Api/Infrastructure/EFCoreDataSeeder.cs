...
internal static class EfCoreDataSeeder
{
    ...
    {
        const int minBudget = (int)Club.LeagueMinimumBudgetRequired;
        ...
        UsedClubsId.AddRange(clubs.Select(c => c.Id));

        await clubs.AddRangeAsync(clubs, ct);
    }
    ...
    private static async Task AddClubContractsAsync(DbContext dbContext, CancellationToken ct)
    {
        for (int i = 0; i < 4; i++)
        {
            PlayerContract contract = new()
            {
                ...
                Duration = DateRange.Create(DateTime.Today, DateTime.Today.AddYears(3)).Value,
        ...
        CoachContract guardiolaContract = new()
        {
            ...
            Duration = DateRange.FromTodayTo(DateTime.Today.AddYears(3)).Value,
            AnualSalary = RandomNumberGenerator.GetInt32(1000, 5000)
        };
