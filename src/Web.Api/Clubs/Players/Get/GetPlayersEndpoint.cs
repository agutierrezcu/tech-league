...
internal sealed class GetPlayersEndpoint
     (IQueryHandler<GetPlayerByClubQuery, GetPlayerByClubQueryResponse> queryHandler)
        : Endpoint<GetPlayersRequest, PlayerByClub[]>
{
    ...
    public override async Task HandleAsync(GetPlayersRequest r, CancellationToken ct)
    {
        ...
        Result<GetPlayerByClubQueryResponse> result = await queryHandler.HandleAsync(query, ct);

        await result.Match(
            v =>
            {
                HttpContext.Response.Headers.AddPaginationHeaders(v);
                return Send.OkAsync(v.Data, cancellation: ct);
            },
